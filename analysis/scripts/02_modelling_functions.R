...
run_model <- function(model_name, stan_model, overwrite, iter = 5e3){
  ...
  if(!file.exists(paste0("analysis/output/model_fits/", model_name, ".rds")) | overwrite){
    
    model_fit <-
      stan(file = paste0("analysis/input/models/", stan_model, ".stan"),
           data = stan_data,
           iter = iter,
           warmup = iter*0.9,
           chains = 3,
    ...
    if(!dir.exists("analysis/output/model_fits/")) dir.create("analysis/output/model_fits/")
    
    write_rds(x = model_fit, 
              file = paste0("analysis/output/model_fits/", model_name, ".rds"))
...
extract_pars <- function(model_name, overwrite){
  ...
  if(!file.exists(paste0("analysis/output/model_pars/", model_name, ".parquet")) | overwrite){
    
    param_table <-
      paste0("analysis/output/model_fits/", model_name, ".rds") %>% 
      read_rds() %>% 
      as_draws_df() %>% 
    ...
    write_parquet(x = param_table, 
                  sink = paste0("analysis/output/model_pars/", model_name, ".parquet"))
    
    write_parquet(x = param_table_summary, 
                  sink = paste0("analysis/output/model_pars/", model_name, "_summary.parquet"))
...
run_traceplots <- function(model_name, overwrite = TRUE){
  
  model_par_list <-
    paste0("analysis/output/model_pars/", model_name, ".parquet") %>% 
    read_parquet() %>% 
  ...
  for(i in 1:nrow(model_par_list)){
    ...
    filename <- paste0("analysis/output/traceplots/", model_name, "/", model_par_list$param[i],".png")
    
    if(!file.exists(filename) | overwrite){
      
      max_plots <- ifelse(model_par_list$n[i] > 20, 20, model_par_list$n[i])
      
      if(model_par_list$n[i] == 1){
        param_name  <- model_par_list$param[i]
      } else {
        param_name <- paste0(model_par_list$param[i], "[", 1:max_plots, "]")
      }
      
      p <-
        traceplot(model_fit, param_name, inc_warmup = FALSE) 
      
      if(!dir.exists("analysis/output/traceplots/")){
        dir.create("analysis/output/traceplots/")
      }
      if(!dir.exists(paste0("analysis/output/traceplots/", model_name, "/"))){
        dir.create(paste0("analysis/output/traceplots/", model_name, "/"))
      }
...
run_ess_check <- function(model_name, overwrite = TRUE){
  
  filename <- paste0("analysis/output/model_checks/", model_name, ".png")
  
  if(!file.exists(filename) | overwrite){
    
    rhat_values <-
      paste0("analysis/output/model_pars/", model_name, "_summary.parquet") %>% 
      read_parquet() %>% 
...
run_model_vis <- function(model_name, obs_data, populations = 1:20, output_filename = NULL, overwrite = TRUE){
  
  outfile <- ifelse(!is.null(output_filename), 
                    output_filename, 
                    paste0("analysis/output/model_plots/", model_name, ".png"))
  ...
  if(!file.exists(outfile) | overwrite){
    ...
    plot <-
      paste0("analysis/output/model_pars/", model_name, ".parquet") %>% 
      read_parquet() %>%
      filter(population_indx %in% populations) %>% 
      pivot_wider(names_from = param, values_from = value) %>% 
      full_join(obs_data %>% filter(population_indx %in% populations), 
...
run_model_vis_continuous <- function(model_name, obs_data, overwrite = TRUE){
  
  filename <- paste0("analysis/output/model_plots/", model_name, ".png")
  ...
  if(!file.exists(filename) | overwrite){
    ...
    plot <-
      ...
      pivot_wider(names_from = param, values_from = value) %>% 
      left_join(obs_data %>% summarise(max_size = max(tl), min_size = min(tl), .by = population_indx)) %>% 
      expand_grid(tibble(size = seq(0, max(obs_data$tl), by = 0.1))) %>% 
...
run_par_regression <- function(model_name, overwrite = TRUE){
  
  filename <- paste0("analysis/output/model_param_plots/", model_name, ".png")
  ...
  if(!file.exists(filename) | overwrite){
    ...
    dat1 <-
      paste0("analysis/output/likelihood_values/", model_name, ".parquet") %>% 
      read_parquet()
    ...
    plot_data <- 
      model_par_raw %>% 
      left_join(point_size) %>% 
      left_join(norm_better_n2)
    
    plot_pars <-  function(data = plot_data, 
                           x = "mean_mu", 
                           y = "mean_sigma"){
    ...
    ggsave(filename = paste0("analysis/output/model_param_plots/", model_name, ".png"), 
           plot = p4,
           height = 10, 
           width = 20*1.618, 
           units = "cm", 
...
run_ll <- function(model_name, overwrite){
  ...
  if(!file.exists(paste0("analysis/output/likelihood_values/", model_name, ".parquet")) | overwrite){
    ...
    if(!dir.exists("analysis/output/likelihood_values/")) dir.create("analysis/output/likelihood_values/")
...
run_ll_plot <- function(model_name, obs_data, overwrite = TRUE){
  
  filename <-paste0("analysis/output/likelihood_plots/boxplot_", model_name, ".png")
  ...
  if(!file.exists(filename) | overwrite){
    ...
    if(!dir.exists("analysis/output/likelihood_plots/")) dir.create("analysis/output/likelihood_plots/")
...
run_mean_vs_ll <- function(model_name, obs_data, overwrite = TRUE){
  
  filename <-paste0("analysis/output/likelihood_plots/sizeVSll_", model_name, ".png")
  ...
  if(!file.exists(filename) | overwrite){
    ...
    ggsave(filename = paste0("analysis/output/likelihood_plots/sizeVSll_", model_name, ".png"), 
           plot = p,
           height = 20, 
           width = 20*1.618, 
           units = "cm", 
    ...
    ggsave(filename = paste0("analysis/output/likelihood_plots/sizeVSll_", model_name, "_method2.png"), 
           plot = p2,
           height = 20, 
           width = 20*1.618, 
           units = "cm", 
...
run_mean_vs_ll_continuous <- function(model_name, obs_data, overwrite = TRUE){
