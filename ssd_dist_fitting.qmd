...
<<<<<<< HEAD
mod_05_allspp_data <- mod_06_allspp_data <- mod_07_allspp_data <-
  mod_08_allspp_data <- mod_09_allspp_data <- mod_10_allspp_data <- 
  mod_11_allspp_data <- mod_12_allspp_data <-
  make_datalist(data_obs_count_allspp, data_obs_count_allspp_byspp)
=======

count_data <-
  read_csv("extra/sim_and_obs_data.csv") %>% 
  left_join(rls_bin_table, 
            by = join_by(size_class, size_min, size_max)) %>% 
  add_count(species_indx, wt = n, name = "totaln_spp") %>% 
  mutate(p = n/totaln_spp) %>% 
  mutate(row = 1:n()) %>% 
  mutate(min_row = min(row), 
...
>>>>>>> ff093c73c7070b483d7b139451d938463dd9633f
...
compare_pars_plot("mod_07_allspp", "mod_08_allspp")
compare_pars_plot("mod_09_allspp", "mod_10_allspp")
compare_pars_plot("mod_11_allspp", "mod_12_allspp")
...
get_ll <- function(species_indx, sigma, mu){
  target = 0
  for (i in mod_11_data$i_min[species_indx]:mod_11_data$i_max[species_indx]) { 
    
    normalisation_const = 1 - pnorm(mod_11_data$l[1], mu, sigma); 
        
      bin_prob = (pnorm(mod_11_data$l[mod_11_data$b[i]+1], mu, sigma) - 
        pnorm(mod_11_data$l[mod_11_data$b[i]], mu, sigma))/normalisation_const; 
        
        bin_prob = max(1e-8, bin_prob);

      target = target + mod_11_data$n[i]*log(bin_prob);
  }
  return(target)
}

get_ll_lnorm <- function(species_indx, sdlog, meanlog){
  target = 0
  for (i in mod_11_data$i_min[species_indx]:mod_11_data$i_max[species_indx]) { 
    
    normalisation_const = 1 - plnorm(mod_11_data$l[1], meanlog, sdlog); 
        
      bin_prob = (plnorm(mod_11_data$l[mod_11_data$b[i]+1], meanlog, sdlog) - 
        plnorm(mod_11_data$l[mod_11_data$b[i]], meanlog, sdlog))/normalisation_const; 
...
par_table <- 
  ...
  pivot_wider(names_from = parameter, 
              values_from = value) %>% 
  mutate(ll = pmap_dbl(.l = list(species_indx = species_indx, 
                                 sigma = sigma, 
                                 mu = mu), 
                       .f = get_ll))
...
par_table2 <- 
  ...
  pivot_wider(names_from = parameter, 
              values_from = value) %>% 
  mutate(ll_lnorm = pmap_dbl(.l = list(species_indx = species_indx, 
                                 sdlog = sdlog, 
                                 meanlog = meanlog), 
                       .f = get_ll_lnorm))
...
ll_plots <- 
  par_table %>%
  left_join(par_table2) %>% 
  left_join(read_csv("extra/the_secret_v2.csv")) %>% 
  # filter(simulated, dist == "N") %>% 
  pivot_longer(cols = c(ll, ll_lnorm)) %>% 
  left_join(data_obs_count_20spp %>% 
              select(species_indx, species_name) %>% distinct()) %>% 
  ggplot(aes(y =  value, 
             x = name, col = name)) +
   scale_color_manual(values=c("red", "blue"))+
    geom_boxplot() +
  facet_wrap(~species_indx, scales = "free",ncol =8)

save_plot("output/figures/ll_plots_all.png",
            ll_plots,  
          base_height = 8)
