...
mod_07_allspp_pars <- extract_pars("mod_07_allspp") %>% filter(quantile == "median")
mod_08_allspp_pars <- extract_pars("mod_08_allspp") %>% filter(quantile == "median")

lm1 <- lm(sigma ~ mu, data = mod_07_allspp_pars)
lm2 <- lm(sdlog ~ meanlog, data = mod_08_allspp_pars)

p4 <- 
  mod_07_allspp_pars %>% 
  mutate(fit = predict(lm1)) %>% 
  ggplot() +
  aes(
    x = mu, 
    y = sigma
  ...
  geom_line(aes(y = fit), 
            col = "blue") +
  theme_cowplot(20) +
  lims(y = layer_scales(p1)$y$range$range, 
       x = layer_scales(p1)$x$range$range)

p5 <- 
  mod_08_allspp_pars %>% 
  mutate(fit = predict(lm2)) %>% 
  ggplot() +
  aes(
    x = meanlog, 
    y = sdlog
  ...
  geom_line(aes(y = fit), 
            col = "red") +
  theme_cowplot(20) +
  lims(y = layer_scales(p2)$y$range$range, 
       x = layer_scales(p2)$x$range$range)

p6 <- p4 + p5

save_plot(filename = "output/figures/mod_07_vs_mod_08_pars.png", 
          plot = p6, 
          base_height = 8, 
          base_asp = 1.618*2)
```
...
mod_09_allspp_pars <- extract_pars("mod_09_allspp") %>% filter(quantile == "median")
mod_10_allspp_pars <- extract_pars("mod_10_allspp") %>% filter(quantile == "median")

lm1 <- lm(sigma ~ mu, data = mod_09_allspp_pars)
lm2 <- lm(sdlog ~ meanlog, data = mod_10_allspp_pars)
...
save_plot(filename = "output/figures/mod_09_vs_mod_10_pars.png", 
          plot = p9, 
          base_height = 8, 
          base_asp = 1.618*2)
...
mod_11_allspp_pars <- extract_pars("mod_11_allspp") %>% filter(quantile == "median")
mod_12_allspp_pars <- extract_pars("mod_12_allspp") %>% filter(quantile == "median")

lm1 <- lm(sigma ~ mu, data = mod_11_allspp_pars)
lm2 <- lm(sdlog ~ meanlog, data = mod_12_allspp_pars)
...
save_plot(filename = "output/figures/mod_11_vs_mod_12_pars.png", 
          plot = p6, 
          base_height = 8, 
          base_asp = 1.618*2)
```
