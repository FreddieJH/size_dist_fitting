...
species_list_new <- unique(newdata$species_name)

species_indx_table_new <- 
  tibble(
    species_name = species_list_new, 
    species_indx = 1:length(species_list_new))
  
data_obs_count_allspp <- 
  newdata %>% 
  left_join(species_indx_table_new, by = join_by(species_name)) %>% 
  left_join(rls_bin_table, by = join_by(size_class)) %>% 
  add_count(species_indx, wt = n, name = "totaln_spp") %>% 
  arrange(species_indx, size_indx) %>%
  mutate(p = n/totaln_spp) %>% 
...
mod_05_data <- mod_06_data <- mod_07_data <-
  ...
  mod_11_data <- mod_12_data <- mod_13_data <-
...
=======
  mod_11_allspp_data <- mod_12_allspp_data <- mod_13_allspp_data <-
>>>>>>> 32607a41e197db8b3a59528d90c9112163654514
...
run_stanmod("mod_13") %>% traceplot("mu")
...
extract_pars("mod_13_allspp", overwrite_pars = FALSE) %>% 
  ggplot(aes(x = cv_N)) +
  geom_density()
...
xx %>% 
  filter(quantile == "median") %>% pull(sdlog) %>% plot()
...
if(!file.exists("output/likelihoods/mod_13_allspp_poplvl.parquet")){
  par_table <- 
  ...
  write_parquet(par_table3, "output/likelihoods/mod_13_allspp_poplvl.parquet")
}

par_table3 <- read_parquet("output/likelihoods/mod_13_allspp_poplvl.parquet")
