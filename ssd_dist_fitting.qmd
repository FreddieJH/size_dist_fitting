...
make_datalist <- function(mod_name){
  ...
  count_data_name <- 
    case_when(mod_name %in% c("mod_01, mod_02") ~ "data_sim", 
              str_detect(mod_name, "_allspp") ~ "data_obs_count_allspp", 
              str_detect(mod_name, "_poplvl") ~ "data_obs_count_poplvl", 
...
xx2 <- run_stanmod("mod_14_poplvl", overwrite = TRUE, init_r = 1)

xx2 %>% traceplot("mu")
xx2 %>% traceplot("sigma")
xx2 %>% traceplot("meanlog")
xx2 %>% traceplot("sdlog")
xx2 %>% traceplot("select_mu")
xx2 %>% traceplot("select_s")
