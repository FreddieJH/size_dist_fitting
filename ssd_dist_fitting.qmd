...
xx <- read_rds("output/model_fits/mod_15_poplvl.rds")
...
model_output <-
draw_summary %>% 
  mutate(population_indx = str_extract(variable, 
                                    "(?<=\\[).*(?=\\])") %>% as.numeric(), 
         param = str_extract(variable, 
                             ".*(?=\\[)")) %>% 
  filter(!str_detect(param, "ln"), 
         !str_detect(param, "lp__")) %>% 
  select(param, 
         population_indx,
         mean, q5, q95) %>% 
  pivot_wider(names_from = param, values_from = c(mean, q5, q95)) %>% 
  left_join(data_obs_poplvl %>% 
              select(species_latlon, species_name, population_indx) %>% distinct()) %>% 
...
data_obs_poplvl %>% 
  ...
  mutate(fit_norm = 
           pnorm(size_max, mean_mu, mean_sigma) -  
           pnorm(size_min, mean_mu, mean_sigma),
         upr_norm = 
           pnorm(size_max, q95_mu, q95_sigma) -  
           pnorm(size_min, q95_mu, q95_sigma),
         lwr_norm = 
           pnorm(size_max, q5_mu, q5_sigma) -  
           pnorm(size_min, q5_mu, q5_sigma),
             fit_lnorm =
           plnorm(size_max, mean_meanlog, mean_sdlog) - 
           plnorm(size_min, mean_meanlog, mean_sdlog),
         upr_lnorm = 
           plnorm(size_max, q95_meanlog, q95_sdlog) -  
           plnorm(size_min, q95_meanlog, q95_sdlog),
         lwr_lnorm = 
           plnorm(size_max, q5_meanlog, q5_sdlog) -  
           plnorm(size_min, q5_meanlog, q5_sdlog)) %>% 
  ...
  geom_line(aes(y = fit_lnorm), 
            col = "blue") +
  geom_ribbon(aes(ymin = lwr_norm, ymax = upr_norm), fill = "red", alpha = 0.3) +
  geom_ribbon(aes(ymin = lwr_lnorm, ymax = upr_lnorm), fill = "blue", alpha = 0.3) +
  geom_point() + 
  geom_line() +

  facet_wrap(~species_latlon)


tibble(x = seq(0, 50, by = 0.1), mu = exp(1.4975), sigma = exp(-0.0475)) %>% 
  mutate(y = plogis(x, mu, sigma)) %>% 
  ggplot(aes(x, y)) +
  geom_line()

xx2 <-
  stan("input/models/mod_15.stan",
...
xx %>% traceplot(paste0("meanlog[", 1:50, "]"))
xx %>% traceplot(paste0("sdlog[", 101:151, "]"))
xx %>% traceplot(paste0("ln_mu[", 1:50, "]"))
xx %>% traceplot(paste0("sigma[", 101:151, "]"))
xx %>% traceplot(paste0("logistic_mu"))
xx %>% traceplot(paste0("logistic_sigma"))
...
poplvl_dat <-
  ...
  clean_data(population = "species_latlon") %>% 
   mutate(species_name = str_extract(species_latlon, ".*(?=__)"), 
         lat = str_extract(species_latlon, "(?<=__).*(?=_)") %>% as.numeric(), 
         lon = str_extract(species_latlon, "(?<=_)(\\d|-)*$") %>% as.numeric()) %>% 
   add_count(species_latlon, size_class, wt = n, name = "weighted_size") %>% 
  mutate(weighted_size/n)
