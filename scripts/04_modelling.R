  ...
  for(pop in c("species", "ecoregion", "gridcell")){
    for(mod in c("normal", "lognormal")){
      for(bin_cbf in c(TRUE, FALSE)){
        ...
        if(bin_cbf & dat == "rls") next
        
        population_list <- 
          get(paste0("obsdata_", dat, "_", pop, ifelse(bin_cbf, "_binned", ""))) %>% 
          pull(population) %>% 
          unique() 
        ...
        for(i in population_list) {
          ...
          filename <- 
            paste0("output/models/fits/", 
                   ifelse(bin_cbf, "cbf_binned/", paste0(dat, "/")),
                   pop, "/",
                   mod, "/",
                   i,
                   ".rds")
            ...
            if (p_firstbin < 0.5) {
              ...
              stan_data <- 
                list(
                  ...
                  low_bound = ifelse(dat == "cbf" & bin_cbf==FALSE, 0.05, 1.25)
                )
      ...
      for(bin_cbf in c(TRUE, FALSE)){
        ...
        diag_filename <- 
          paste0("output/models/diagnostics/", 
                 ifelse(bin_cbf, "cbf_binned_", paste0(dat, "_")),
                 pop, "_",
                 mod, "_diags.csv")
      ...
      for(bin_cbf in c(TRUE, FALSE)){
        ...
        summary_filename <- 
          paste0("output/models/summary/posteriors/", 
                 ifelse(bin_cbf, "cbf_binned", dat), "_", 
                 pop, "_",
                 mod,  
                 ".parquet")
...
calc_nc_norm <- Vectorize(function(mu, sigma, lowbound) 1 - integrate(dnorm, lower = -500, upper = lowbound, mean = mu, sd = sigma)$value)
calc_mean_norm <- Vectorize(function(mu, sigma, nc, lowbound) integrate(f_mean_norm, lower = lowbound, upper = 500, mu = mu, sigma = sigma, nc = nc)$value)
calc_var_norm <- Vectorize(function(mu, sigma, nc, mean, lowbound) integrate(f_var_norm, lower = lowbound, upper = 500, mu = mu, sigma = sigma, nc = nc, mean = mean)$value)
# 
...
quant_rmna <- function(x, p) quantile(x, p, na.rm = TRUE)
  ...
  for(pop in c("gridcell", "species", "ecoregion")){
    for(mod in c("lognormal", "normal")){
      for(bin_cbf in c(TRUE, FALSE)){
        ...
        lower_bound <- ifelse(dat == "cbf" & bin_cbf==FALSE, 0.05, 1.25)
            ...
            input_filename %>% 
              ...
              mutate(nc = calc_nc_norm(lowbound = lower_bound, 
                     ...
                     var = calc_var_norm(lowbound = lower_bound,
                                         ...
                                         mean = mean),
                     cv = sqrt(var)/mean) %>%
              summarise(mean_05 = quant_rmna(mean, 0.05),
                        mean_10 = quant_rmna(mean, 0.1),
                        mean_50 = quant_rmna(mean, 0.5),
                        mean_90 = quant_rmna(mean, 0.9),
                        mean_95 = quant_rmna(mean, 0.95),
                        cv_05 = quant_rmna(cv, 0.05),
                        cv_10 = quant_rmna(cv, 0.1),
                        cv_50 = quant_rmna(cv, 0.5),
                        cv_90 = quant_rmna(cv, 0.9),
                        cv_95 = quant_rmna(cv, 0.95),
                        logl_50 = quant_rmna(lp__, 0.5), 
                        .by = population) %>%
...
# 
# for(i in 1:nrow(xx)){
#   calc_nc_norm(lowbound = lower_bound, 
#                mu = xx[i,]$mu,
#                sigma = xx[i,]$sigma)
# }
      ...
      for(bin_cbf in c(TRUE, FALSE)){
        ...
        if(!file.exists(output_filename)|force_run){
          ...
          if(mod == "normal"){
            input_filename %>% 
              read_parquet() %>% 
              summarise(mu_05 = quant_rmna(mu, 0.05),
                        mu_10 = quant_rmna(mu, 0.1),
                        mu_50 = quant_rmna(mu, 0.5),
                        mu_90 = quant_rmna(mu, 0.9),
                        mu_95 = quant_rmna(mu, 0.95),
                        sigma_05 = quant_rmna(sigma, 0.05),
                        sigma_10 = quant_rmna(sigma, 0.1),
                        sigma_50 = quant_rmna(sigma, 0.5),
                        sigma_90 = quant_rmna(sigma, 0.9),
                        sigma_95 = quant_rmna(sigma, 0.95),
                        logl_50 = quant_rmna(lp__, 0.5), 
                        .by = population) %>%
              write_parquet(output_filename)
          ...
          if(mod == "lognormal"){
            ...
            input_filename %>% 
              read_parquet() %>%
              summarise(meanlog_05 = quant_rmna(meanlog, 0.05),
                        meanlog_10 = quant_rmna(meanlog, 0.1),
                        meanlog_50 = quant_rmna(meanlog, 0.5),
                        meanlog_90 = quant_rmna(meanlog, 0.9),
                        meanlog_95 = quant_rmna(meanlog, 0.95),
                        sdlog_05 = quant_rmna(sdlog, 0.05),
                        sdlog_10 = quant_rmna(sdlog, 0.1),
                        sdlog_50 = quant_rmna(sdlog, 0.5),
                        sdlog_90 = quant_rmna(sdlog, 0.9),
                        sdlog_95 = quant_rmna(sdlog, 0.95),
                        logl_50 = quant_rmna(lp__, 0.5), 
                        .by = population) %>%
              write_parquet(output_filename)
          }
