...
#       
#       files <- 
#         list.files("output/models/cv/",
#                    pattern = paste0(dat, "_", 
#                                     pop, "_",
#                                     mod, "_"), 
#                    full.names = TRUE)
#       
#       outname <- 
#         paste0("output/models/summary/",
#                dat, "_", 
#                pop, "_",
#                mod, 
#                "_cv.csv"
#         )
#       
#       if(!file.exists(outname)|force_run){
#         outdat <- tibble()
#         count = 0
#         for(j in files){
#           count = count+1
#           curr_pop <- str_extract(j, "([^\\//]+(?=.parquet))") %>% 
#             str_remove(paste0(dat, "_", 
#                               pop, "_",
#                               mod, "_"))
#           outdat <-
#             outdat %>% 
#             bind_rows(read_parquet(j) %>% 
#                         mutate(population = curr_pop))
#           
#           print(paste0("(", count, ") ", curr_pop, " added."))
#         }
#         
#         write_csv(outdat, 
#                   outname)
#         rm(outdat)
#       } else {
#         print(paste(outname, "already saved."))
#       }
#       
#       rm(mod)
#     }
...
# 
# my_results <- 
#   read_csv("output/models/summary/rls_gridcell_normal_cv.csv") %>% 
#   mutate(mod = "normal") %>% 
#   bind_rows(read_csv("output/models/summary/rls_gridcell_lognormal_cv.csv") %>% 
#               mutate(mod = "lognormal")) %>% 
#   pivot_wider(values_from = contains("_"), 
#               names_from = mod) 
# 
# shanes_results <- 
#   read_csv("results.csv") %>% 
#   rename(popindx = `pop <- 1:MAX_POP`) %>% 
#   left_join(popindx_tbl)
# 
# left_join(shanes_results, my_results, by = join_by(population)) %>% 
#   ggplot(aes(x = mu50_nr, 
#              y = mean_50_normal)) +
#   geom_point() +
#   geom_abline(slope = 1)
# 
# best_model <- 
#   read_csv("output/models/summary/rls_gridcell_normal_cv.csv") %>% 
#   select(population, norm_ll = logl_50) %>% 
#   full_join(read_csv("output/models/summary/rls_gridcell_lognormal_cv.csv") %>% 
#               select(population, lnorm_ll = logl_50)) %>% 
#   mutate(normal_better = norm_ll > lnorm_ll) %>% 
...
#   drop_na() %>% 
#   ggplot(aes(x = mean_50, y = cv_50, col = normal_better)) + 
#   # geom_errorbarh(aes(y = cv_50, xmin = mean_05, xmax = mean_95), color = "grey90") +
#   # geom_errorbar(aes(x = mean_50 , ymin = cv_05, ymax = cv_95), color = "grey90") +
...
#   bind_rows(  read_csv("output/models/summary/rls_gridcell_lognormal_cv.csv") %>% 
#                 mutate(model = "lnorm")) %>% 
#   left_join(best_model) %>% filter(cv_50 < 0.75, cv_50 > 0.65) %>% view()
# 
# 
# read_csv("output/models/summary/rls_gridcell_normal_cv.csv") %>% 
#   mutate(model = "norm") %>% 
...
#                 mutate(model = "lnorm")) %>% 
#   left_join(best_model) %>% 
#   ggplot(aes(x = cv_50, col = normal_better)) + 
#   geom_density()
# 
# read_csv("output/models/summary/rls_gridcell_normal_cv.csv") %>% 
#   mutate(model = "norm") %>% 
...
# x2 %>% filter(is.na(par))
# 
# out1 <- read_csv("output/model_diags_all.csv")
# out2 <- read_csv("output/model_pars_all.csv")
# 
# out1 %>% filter(rhat > 1.1)
# out1 %>% arrange(n_eff)
# 
# old <- read_parquet("old/output/model_fits/rls_pars_normal_gridcell.parquet")
# old_summary <- 
#   old %>% 
#   summarise(mu_old = median(mu), 
#             sigma_old = median(sigma), 
#             .by = population)
# new_summary <- 
#   out2 %>% 
#   summarise(mu_new = median(mu), 
#             sigma_new = median(sigma), 
#             .by = population)
# 
# 
# 
...
# traceplot(fit)
# 
# x2 <- readRDS("output/model_fits/rls/gridcell/Atypichthys strigatus__-33_152.rds")
# x2
# traceplot(x2)
# 
# 
# files <- list.files("output/model_fits/rls/gridcell/", full.names = TRUE)
# 
# 
# 
# files2 <- list.files("output/model_diagnostics", full.names=TRUE, pattern = ".csv")
# 
# out <- tibble()
# for(k in files2){
#   
#   curr_pop <- str_extract(k, "(?<=model_diagnostics/)(.*)(?=.csv)")
#   
#   out <- 
#     out %>% 
#     bind_rows(
#       read_csv(k, show_col_types = FALSE) %>% 
#         mutate(population = curr_pop)
#     )
#   
# }
# 
# write_csv(out, "output/model_diags_all.csv")
# 
# 
# 
# files3 <- list.files("output/posteriors", full.names=TRUE, pattern = ".csv")
# 
# out2 <- tibble()
# for(l in files3){
#   
#   curr_pop <- str_extract(l, "(?<=posteriors/)(.*)(?=.csv)")
#   
#   out2 <- 
#     out2 %>% 
#     bind_rows(
#       read_csv(l, show_col_types = FALSE) %>% 
#         mutate(population = curr_pop)
#     )
#   
# }
# 
# write_csv(out2, "output/model_pars_all.csv")
# 
# 
# 
# 
# 
# str_extract("output/model_fits/rls/gridcell/Dascyllus trimaculatus__-14_122.rds", "(?<=gridcell/)(.*)(?=.rds)")
# 
# obsdata_rls_gridcell$population %>% unique() %>% length() # 3091 populations
# 
# abundant_first_bin <- 
#   obsdata_rls_gridcell %>% 
#   mutate(min_size_class = min(size_class), .by = population) %>% 
#   filter(p_obs > 0.5 & size_class == min_size_class) %>% 
#   pull(population) %>% 
#   unique()
# 
# 3091 - length(abundant_first_bin) # 3091 - 152 = 2939 populations
# 
# out1 <- read_csv("output/model_diags_all.csv")
# out2 <- read_csv("output/model_pars_all.csv")
# 
...
# 
# 
# left_join(old_summary, new_summary) %>% 
#   filter(mu_old > 2.5) %>%
#   pivot_longer(cols = -population) %>% 
#   ggplot(aes(x = value, 
#              col = name, 
#              alpha = 0.1))  +
#   geom_density(linewidth = 2, alpha = 0.1) 
# 
# 
# left_join(old_summary, new_summary) %>% 
#   ggplot(aes(x = sigma_old, 
#              y = sigma_new)) +
#   geom_point() 
# 
# 
# readRDS("output/model_fits/rls/gridcell/Chromis margaritifer__-23_114.rds") %>% 
#   traceplot()
