...
for(min_count in c(500, 100, 50)){
  for(population_level in c("location")){
...
for(min_count in c(500, 100, 50)){
  ...
  stan_data <- 
    ...
    run_traceplots(model_name = model_name)
    run_ess_check(model_name = model_name)
    run_model_vis_continuous(model_name = model_name, obs_data = obs_data)
    run_par_regression(model_name = model_name)
    run_ll_continuous(model_name = model_name, overwrite = TRUE)
    run_ll_plot(model_name = model_name, obs_data = obs_data)
    run_mean_vs_ll_continuous(model_name = model_name, obs_data = obs_data)
  }
}
