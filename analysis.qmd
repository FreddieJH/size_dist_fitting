...
fig1_data   <- read_csv("fig_data/obsdata_population_scaled.csv")
fig1b_data   <- read_parquet("fig_data/fig1b_data.parquet")
fig2_data_pop    <- read_csv("fig_data/fig2_data_pop.csv")
fig2_data_spp    <- read_csv("fig_data/fig2_data_spp.csv")
...
SI_fig1_data <- read_csv("fig_data/figS1_data.csv")
SI_fig3_data <- read_csv("fig_data/figS3_data.csv")
SI_fig4_data <- read_csv("fig_data/figS4_data.csv")
SI_fig5_data <- read_csv("fig_data/figS5_data.csv")
SI_fig7_data <- read_csv("fig_data/figS7_data.csv")
SI_fig9_data <- read_csv("fig_data/figS9_data.csv")
...
fig1_data_gg <- 
  ...
  mutate(col = case_when(
    ...
    alpha = 0.03, 
    type = "obs") %>% 
  select(scaled_n, scaled_size, col, lwd, population, alpha, type) %>% 
  bind_rows(fig1_model_fits %>% mutate(population = dist, 
                                       alpha = 1, 
                                       type = "fit") %>% 
              select(scaled_n, scaled_size, col, lwd, alpha, type)) %>% 
...
fig1a <-  
  ...
  guides(col = guide_legend(
    override.aes = list(colour = c(norm_colour,
                                   lnorm_colour,
                                   norm_colour,
                                   lnorm_colour),
                        linewidth = c(1,1,3,3)), 
...
saveRDS(fig1a, "fig_data/fig1a.rds")
...
fig1b <- 
  ...
  mutate(dist = case_when(dist == "norm" ~ "normal", 
                          dist == "lnorm" ~ "lognormal")) %>%
  filter(dist == better_dist) %>% 
  ggplot(aes(x = size, y = n, group = population, col = dist)) +
  geom_line(alpha = 0.03) +
    geom_line(aes(x = scaled_size, 
      y = scaled_n, group = col), data = fig1_data_gg %>% filter(type != "obs"), inherit.aes = FALSE) +
  scale_x_continuous(label = label_number(suffix = "x"), 
                     expand = c(0.01,0),
                     limit = c(NA, 4)) +
  scale_color_manual(guide = "legend", 
                     ...
                     labels = c("normal" = "Normal fit (population-level)", 
                                "lognormal" = "Lognormal fit (population-level)")) +
   labs(
    x = "Body length relative to mean length",
    y = "Abundance relative to max"
  ) +
  ...
  theme(legend.position = c(1,1), 
        ...
        legend.background = element_rect(fill = "transparent")) +
  guides(col = guide_legend(
    override.aes = list(linewidth = c(1,1), 
                        alpha = c(1,1)), 
    label.position = "left"))
...
fig1 <- 
  wrap_elements(
    fig1a + theme(axis.text.x = element_blank(),
                  axis.title = element_blank()) + 
      fig1b + theme(axis.title.y = element_blank()) +
      plot_layout(ncol = 1) +
      theme(plot.margin = margin(5.5, 5.5, 5.5, 0))
  ) +
  labs(tag = "Abundance relative to max") +
  theme(
...
ggsave(filename = "ms_figs/scaling_distributions_v2.png",
       plot = fig1,
       height = 20,
       width = 20,
       units = "cm", 
...
make_plot <- function(data, title){
  data %>% 
    ...
    geom_path(aes(y = p_lnorm), linewidth = 2, 
              col = lnorm_colour) +
    geom_path(aes(y = p_norm), linewidth = 2, 
              col = norm_colour) +
    scale_alpha_identity()+
...
fig3_data_all %>% filter(species == "Neopomacentrus azysron") %>% view()
fig3_data_all <- read_csv("fig_data/fig3_data_all.csv")
for(spp in unique(fig3_data_all$species)){
  for(dat in c("rls", "cbf")){
    ...
    if(nrow(spp_data) == 0) next
    
    make_plot(data = spp_data, 
              title = paste0(
                "data = ", dat,
                "mu = ", unique(spp_data$mu) %>% round(1),
                ", sigma = ", unique(spp_data$sd)%>% round(1), 
                ", meanlog = ", unique(spp_data$meanlog)%>% round(1), 
                ", sdlog = ", unique(spp_data$sdlog)%>% round(1)
              )) %>% 
      ggsave(filename = paste0("ms_figs/spp_dists/", dat, "_", spp, ".png"),
             height = 20,
             width = 20*1.618,
             units = "cm")
  }
...
ks_curve <- 
  ...
  scale_color_manual(values = c("norm_ks" = norm_colour,
                                "lnorm_ks" = lnorm_colour, 
                                "pref_ks" = "pink"),
...
make_fig4 <- function(data, spp_select){
  ...
  dat1 %>% 
    ...
    scale_color_manual(values = c("p_norm" = norm_colour, 
